...
def user_by_anonymous_id(uid):
    ...
    log.error('models 1 {}'.format(uid))
    if uid is None:
        return None

    try:
        v =  User.objects.get(anonymoususerid__anonymous_user_id=uid)
        log.error('models {}'.format(v))
        return v
...
class CourseEnrollment(models.Model):
    ...
    def enroll(cls, user, course_key, mode=None, check_access=False):
        ...
        if check_access:
            if cls.is_enrollment_closed(user, course) and not University2035Block.can_enroll(user, course_key):
                log.warning(
...
class CourseEnrollmentAllowed(DeletableByUserValue, models.Model):
    ...
    def may_enroll_and_unenrolled(cls, course_id):
        ...
        """
        log.error(' def may_enroll_and_unenrolled {}'.format(course_id))
