...
from stsos_widget.models import stsos_info
...
def course_about(request, course_id):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        try:
            can_enroll_2035 = University2035Block.can_enroll(request.user, str(course_id))['can_enroll']
        except Exception:
            can_enroll_2035 = False

        can_enroll = bool(has_access(request.user, 'enroll', course) or can_enroll_2035)
        ...
        info = stsos_info(request.user, course_id)
        if info:
            course_item_url, cabinet_course_url, user_id, visitors_rating, can_rate = info
        else:
            course_item_url, cabinet_course_url, user_id, visitors_rating, can_rate = [0, 0, 0, None, False]
